// ─── Blog Post Data ───────────────────────────────────────────────────────────
// When backend is ready, this will be fetched from /api/blog
const blogPosts = [
  {
    id: 1,
    title: "The Art of Hand Carving: A Beginner's Guide",
    slug: "art-of-hand-carving",
    excerpt: "Discover the tools, techniques, and patience required to start your wood carving journey.",
    date: "2024-03-10",
    author: "Amara Osei",
    category: "Techniques",
    readTime: "5 min read",
    images: ["/images/hand-craving.jpg"],
    content: `
Wood carving is one of humanity's oldest and most rewarding crafts. Whether you're drawn to the meditative rhythm of a chisel on wood or the satisfaction of transforming a raw block into something beautiful, carving is a skill anyone can learn.

## Getting Started: The Essential Tools

Before you pick up a piece of wood, you need the right tools. For beginners, you don't need much:

- **A bench knife** – for whittling and shaping smaller pieces
- **A gouge** – a curved chisel perfect for scooping and hollowing
- **A V-tool** – for cutting sharp lines and details
- **A mallet** – used with larger chisels for harder woods

Always start with sharp tools. A dull blade requires more force and is actually more dangerous than a sharp one.

## Choosing Your Wood

Beginners should start with softer woods that are easy to carve:

- **Basswood** – the most popular choice, smooth grain, easy to cut
- **Butternut** – slightly harder, beautiful natural color
- **White pine** – very soft, great for practice

Avoid hardwoods like oak or teak until you've built up your skills and confidence.

## Your First Project

Start with a simple relief carving — a flat piece of wood with a design carved into the surface. Trace a simple leaf or geometric pattern onto the wood, then use your V-tool to cut along the outline. Gradually deepen and refine the cuts.

Don't rush. Wood carving rewards patience. The greatest artisans at BoraBora Woodcrafts say the same thing: *"Listen to the wood, and it will tell you where to go."*

## Safety Tips

- Always carve away from your body
- Keep your non-carving hand behind the blade
- Use a carving glove on your holding hand
- Secure your work to a bench when possible

The journey of a thousand carvings begins with a single cut. Happy carving!
    `.trim(),
  },
  {
    id: 2,
    title: "Why We Choose Sustainably Sourced Timber",
    slug: "sustainable-timber",
    excerpt: "Our commitment to the environment starts with where we get our wood.",
    date: "2024-02-22",
    author: "Zawadi Mwangi",
    category: "Sustainability",
    readTime: "4 min read",
    images: ["/images/suitability.jpg"],
    content: `
At BoraBora Woodcrafts, sustainability isn't a buzzword — it's the foundation of everything we do. Every piece of wood that enters our workshop has a story, and we make sure it's a responsible one.

## The Problem with Unregulated Timber

East Africa's forests are under enormous pressure. Illegal logging, charcoal production, and land clearing have decimated forest cover across Kenya, Uganda, and Tanzania. Many furniture manufacturers source cheap timber without asking where it comes from.

We refuse to be part of that chain.

## Our Sourcing Standards

We work exclusively with certified suppliers who follow strict forest management practices:

- **FSC-certified forests** – the gold standard for responsible forestry
- **Community woodlots** – local communities planting and harvesting their own timber sustainably
- **Reclaimed and salvaged wood** – giving new life to old timber from demolished buildings

## The Tree Planting Pledge

For every 10 products sold, BoraBora plants one tree in partnership with local reforestation programs in the Aberdare Range and Mount Kenya foothills. Since 2018, we've planted over 2,400 trees.

## What This Means for You

When you buy a BoraBora product, you're not just getting a beautiful piece of craftsmanship. You're supporting:

- Local artisan livelihoods
- Responsible forest stewardship
- A reforestation movement across East Africa

The wood in your home has roots — and we want those roots to keep growing.
    `.trim(),
  },
  {
    id: 3,
    title: "How to Care for Your Wooden Kitchenware",
    slug: "care-for-wooden-kitchenware",
    excerpt: "Simple tips to keep your wooden bowls and boards looking great for years.",
    date: "2024-01-15",
    author: "Fatuma Ali",
    category: "Product Care",
    readTime: "3 min read",
    images: ["/images/wooden-kitchenware.jpg"],
    content: `
Wooden kitchenware is beautiful, functional, and long-lasting — but only if you care for it properly. With a few simple habits, your BoraBora bowls, boards, and utensils will stay gorgeous for decades.

## The Golden Rules

### 1. Never soak in water
Wood and standing water are enemies. Never leave wooden items submerged in the sink or in the dishwasher. The wood will warp, crack, and split.

### 2. Hand wash only
Wash with warm soapy water, then dry immediately with a cloth. Don't leave it on a drying rack — dry it right away.

### 3. Oil regularly
Every 4–6 weeks (more if the wood looks dry), apply a thin coat of **food-grade mineral oil** or **beeswax conditioner**. Let it soak in for a few hours, then wipe off the excess.

This prevents cracking and keeps the wood looking rich and vibrant.

## Removing Odours and Stains

- **Garlic/onion smell**: Rub with half a lemon, leave for 5 minutes, rinse
- **Deep stains**: Mix coarse salt with lemon juice and scrub gently
- **Discolouration**: Light sanding with fine-grit sandpaper, then re-oil

## When to Replace

A well-maintained wooden board or bowl can last a lifetime. If you see deep cracks that trap food, or if the wood has become very soft and spongy, it's time to retire it.

Take care of your wood, and it will take care of you.
    `.trim(),
  },
];

export default blogPosts;
